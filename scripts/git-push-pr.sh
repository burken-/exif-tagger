...
if ! git push -u origin "$FEATURE_BRANCH" --force-with-lease 2>&1; then
    ...
    # Try with an incremented suffix up to 5 times, preserving the original prefix.
    PREFIX="${FEATURE_BRANCH%%/*}"
    for i in $(seq 1 5); do
        SUFFIXED="${PREFIX}/${FEATURE_BRANCH#*/}-try${i}"
    ...
    # If we never updated FEATURE_BRANCH (all retries failed), report error.
...
---
*Generated by \`git-push-pr.sh\` on $(date +%Y-%m-%dT%H:%M:%S%z)*
EOF
)
